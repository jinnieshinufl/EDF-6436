...
my.data <- data.frame(id = c(1:250), 
                      ...
                      gpa_post = rtruncnorm(n=250, a=1.57, b =5.45, mean=3.43, sd= 0.73),
...
```{r mode, exercise=TRUE, echo=FALSE}
...
print(getmode(my.data$age))
hist(my.data$age)
```

```{r mean, exercise=TRUE, echo=FALSE}

print(mean(my.data$gpa_pre))
hist(my.data$gpa_pre)

```


```{r median, exercise=TRUE, echo=FALSE}
x <- c(2,3,3,3,4,4,5,7,7,7,8,8,10,10,11,12,14,14,14)
print(median(x))

print(median(my.data$age))
...
This indicates the **sum of each indivudal's squared deviation from the mean**
 $$ SS = \sum_{i = 1}^{N}(x_i - \bar{X})^2 $$
---

**[CLASS EXERCISE]**: Let's compute the Sum of Squares for *GAP-Post*!

```{r SS, echo=FALSE, exercise=TRUE}
# first we will create a list that substracts the mean from each data point of GPA-Post
...
```{r Z_score, echo=FALSE, exercise=TRUE}
...
- Q1: Calculate the mean of the item 1 variable and interpret it with respect to item difficulty 
- Q2: Calculate the variance and standard deviation of the test score variable 
...
```{r exercise, echo=FALSE, exercise=TRUE}
# Q1 
...
- Coariance values are difficult to interpret because they are dependent on the scale of variables. 
...
```{r exercise2, echo=FALSE, exercise=TRUE}
...
![Zainudin et al. (2014)](https://www.researchgate.net/profile/Siwar-Chamhuri/publication/275530078/figure/tbl1/AS:613863351410722@1523367836730/Pearson-Correlation-Results.png){ width=75% }

- Q1: Describe the direction and statistical significance of three of the correlation coefficients. 
- Q2: Calculate R2 for each of the three correlations and interpret.

```{r corr2, echo=FALSE, exercise=TRUE}
...
## 10. Matrix Algebra 
- In measurement, we are often interested in **multivariate** relationships
- In other words, we will be relating **multiple sets** of scores
- **Matrices** will be used for these multivariate techniques


```{r matrix, echo=FALSE,exercise=TRUE}
# Matrix 
matrix_a = matrix(1:9, nrow = 3, ncol = 3)

# Row vector 
row_vector_a = matrix_a[1,]  # selects all elements of the first column.

# Column vector 
row_vector_a = matrix_a[,1]  # selects all elements of the first column.

# Scalar 
scalar = matrix_a[1,1] #first column, first row 
```
