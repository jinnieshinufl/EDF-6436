...
```{r regression11, echo=FALSE}
fit.reg <-lm(gpa_post ~ gpa_pre, my.data)
```

```{r regression2, echo=FALSE, exercise=TRUE}
fit.reg <-lm(gpa_post ~ gpa_pre, my.data)
...
Let's plot this! 
```{r, plot1, echo=FALSE, exercise=TRUE}
...
```{r, plot2, echo=TRUE, exercise=TRUE}
...
question("What is the predicted value of Y when X = 0?
",
  answer("4.494e-16", correct = TRUE),
  answer("1.196e-01"),
  answer("4.111e-02"),
  answer("2.2e-16")
)
...
- the typical amount from which a statistic from a particular sample size deviates from its analogous parameter
