...
```{r, normal, echo=FALSE, exercise=TRUE}
data = cognition$vocab
...
Q-Q Plot 
```{r, normal2, echo=FALSE, exercise=TRUE}
data = cognition$vocab
ggqqplot(data)

```

```{r, norm3, exercise=TRUE, echo=FALSE}
data = cognition$vocab
shapiro.test(data)
```
...
#library(EFAtools) #
# Assumption 1: Data Sphericity 
#### Bartlett's test of sphericity
BARTLETT(cognition)

# Assumption 2: Sampling Aqequacy 
...
```{r, corr2, echo=TRUE, exercise=TRUE}
one_factor <- fa(cognition, nfactors = 1, fm = "pa", rotate = "promax")

one_factor 

assign("one_factor", one_factor, envir=globalenv()) 
```
...
```{r, gl, exercise=TRUE, echo=FALSE}
datF <- data.frame(lapply(data, ordered))
summary(datF)
...
Factor_scores <- factor.scores(data[,1:32], fafit)$scores
head(Factor_scores)
``` 
